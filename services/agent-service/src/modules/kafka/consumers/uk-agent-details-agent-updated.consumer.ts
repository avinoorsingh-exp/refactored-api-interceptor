...
/**
 * UK Agent Details Agent Updated Consumer
 * 
 * Consumes messages from the UK_AgentDetails_AgentUpdated_V2 topic.
 * 
 ...
 */
@Injectable()
export class UkAgentDetailsAgentUpdatedConsumer implements RegisterableKafkaService {
	private readonly topic = 'UK_AgentDetails_AgentUpdated_V2';
	...
	constructor(
		...
		private readonly ukAgentUpsertService: UkAgentUpsertService,
	...
	async start(): Promise<Consumer> {
		try {
			...
			this.logger.info('UK Agent Details Agent Updated consumer started successfully');
	...
	private async handleMessage(
		...
		this.logger.setContext('UkAgentDetailsAgentUpdatedConsumer');
		try {
	 ...
	 * Translates the Kafka message and upserts agent with associations (modifiedBy: Uk).
	 */
	private async processAgentUpdate(message: unknown): Promise<void> {
		...
		await this.ukAgentUpsertService.upsertAgentWithAssociations(translated);
	}

	/**
	 * Translate UK_AgentDetails_AgentUpdated_V2 Kafka message to database upsert format
	 */
