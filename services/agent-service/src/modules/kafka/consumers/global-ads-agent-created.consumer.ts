...
/**
 * Global ADS Agent Created Consumer
 * 
 * Consumes messages from the Global_ADS_AgentCreated_V2 topic.
 * 
 ...
 */
@Injectable()
export class GlobalAdsAgentCreatedConsumer implements RegisterableKafkaService {
	private readonly topic = 'Global_ADS_AgentCreated_V2';
	...
	constructor(
		...
		private readonly gadsAgentUpsertService: GadsAgentUpsertService,
	...
	private async processMessageWithRetry(
		...
		for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
			try {
				await this.processAgentCreate(message);
	 ...
	 * Translates the Kafka message and upserts agent with associations (modifiedBy: Gads).
	 */
	private async processAgentCreate(message: unknown): Promise<void> {
		...
		await this.gadsAgentUpsertService.upsertAgentWithAssociations(translated);
	}

	/**
	 * Translate Global_ADS_AgentCreated_V2 Kafka message to database upsert format
	 */
	private translateKafkaMessageToUpsertData(payload: any): {
		...
		// Translate agent core fields (InactiveDate → terminationDate when agent is terminated)
		const agent = {
