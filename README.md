...
Coverage reports are generated by the [centralized-coverage Jenkins job](https://jenkins.exprealty.com/job/Transaction%20Calculator/job/centralized-coverage/) on each build.

| Package | Report Location |
|---------|-----------------|
| **agent-service** | Jenkins → Build → Coverage Report |
| **orchestrator** | Jenkins → Build → Coverage Report |
| **@exprealty/database** | `packages/database/coverage/` |
| **@exprealty/shared-domain** | `packages/shared-domain/coverage/` |
| **@exprealty/logger** | `packages/logger/coverage/` |
| **@exprealty/config** | `packages/config/coverage/` |
| **@exprealty/cache** | `packages/cache/coverage/` |
| **@exprealty/api-monitoring** | `packages/api-monitoring/coverage/` (when `pnpm test:cov` is run in that package) |
...
| **Framework** | NestJS | ^11.0.1 | Microservices framework (services and `@exprealty/api-monitoring` aligned on v11) |
...
│   ├── api-monitoring/         # Publishable Nest module: HTTP logging, metrics, admin routes (CodeArtifact)
...
- `@exprealty/api-monitoring`: API monitoring entities (re-exported for the app data source; see that package for the canonical definitions)

### Package: @exprealty/api-monitoring

**Purpose:** NestJS module for HTTP API monitoring (request logs, actor attribution, route metrics, read-only admin endpoints) against **PostgreSQL** and **TypeORM 0.3**.  
**Type:** Publishable workspace package (no other `@exprealty/*` runtime dependencies)  
**Exports:** ESM + TypeScript declarations from `dist/`

**Documentation:** [packages/api-monitoring/README.md](./packages/api-monitoring/README.md) — install, `ApiMonitoringModule.forRoot`, database schema in `core`, and local test commands.

**Publish to AWS CodeArtifact** (after [AWS CLI CodeArtifact login](https://docs.aws.amazon.com/codeartifact/latest/ug/npm-auth.html) for the `npm-packages` repository):

```bash
# From repo root (agent-service)
pnpm run publish:api-monitoring
```

Bump `version` in `packages/api-monitoring/package.json` before each release; `prepublishOnly` runs the build.
